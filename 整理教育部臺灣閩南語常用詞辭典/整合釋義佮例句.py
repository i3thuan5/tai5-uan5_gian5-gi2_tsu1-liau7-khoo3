...
from 整理教育部臺灣閩南語常用詞辭典.教育部閩南語常用詞辭典 import 主編號揣釋義
...
from 整理教育部臺灣閩南語常用詞辭典.教育部閩南語常用詞辭典 import 用釋義揣例句
from 臺灣言語資料庫.腔口資訊 import 閩南語
from 臺灣言語資料庫.欄位資訊 import 袂當替換
from 整理教育部臺灣閩南語常用詞辭典.教育部閩南語常用詞辭典 import 設定詞性
from 整理教育部臺灣閩南語常用詞辭典.教育部閩南語常用詞辭典 import 詞性對照表
from 整理教育部臺灣閩南語常用詞辭典.教育部閩南語常用詞辭典 import 用流水號揣腔口
from 整理教育部臺灣閩南語常用詞辭典.教育部閩南語常用詞辭典 import 用流水號揣地區
from 臺灣言語資料庫.欄位資訊 import 文字組合符號
from 臺灣言語資料庫.欄位資訊 import 會當替換
from 整理教育部臺灣閩南語常用詞辭典.教育部閩南語常用詞辭典 import 文字無音設定
...
class 整合釋義佮例句:
	...
	譀鏡 = 物件譀鏡()
	顯示資料 = False
	def 建立釋義(self, 主編號, 臺語編修物件集, 例句會當變動符號):
...
# 			print(釋義編號)
			if 主編號 == 5843:
				if 義項順序 == 0:
					...
					continue
			釋義文字物件 = 文字.objects.create(來源=教育部閩南語辭典名,
				種類=語句, 腔口=國語, 地區=教育部閩南語辭典地區, 年代=教育部閩南語辭典年代,
				型體=釋義).流水號
			例句資料 = 用釋義揣例句(釋義編號)
# 			臺語編修物件集 = 流水號集  # 用主碼號揣流水號(主編號)
			if self.顯示資料:
				print('臺語編修物件集', 臺語編修物件集)
			for 例句, 標音, 例句翻譯 in 例句資料:
				if 例句翻譯 == '':
					例句翻譯 = 例句
				國語例句物件 = 文字.objects.create(來源=教育部閩南語辭典名,
					種類=語句, 腔口=國語, 地區=教育部閩南語辭典地區, 年代=教育部閩南語辭典年代,
					型體=例句翻譯).流水號
				if 標音[0].isupper():
					種類 = '語句'
...
# 					print('有換音',釋義編號)
				...
				可能音標 = None
				查著資料 = []
				for 臺語編修物件 in 臺語編修物件集:
					文字資料 = 臺語編修物件.文字.all()[0]
					if 文字資料.腔口.startswith(閩南語):
						可能漢字 = 文字資料.型體
						可能音標 = 文字資料.音標
...
# 										漢字頭前, 音標頭前, 漢字後壁, 音標後壁)
				...
				if 例句會當變動符號 and (例句內底漢字 == None or 例句內底音標 == None):
					(例句內底漢字, 例句內底音標) = self.例句變動符號(標準例句, 標準標音, 臺語編修物件集)
...
# 						.format(標準例句, 標準標音, 查著資料),例句內底音標,例句內底漢字)
				for 臺語編修物件 in 臺語編修物件集:
					文字資料 = 臺語編修物件.文字.all()[0]
					if not 文字資料.腔口.startswith(閩南語):
						continue
					
					if 詞性==0:
						關係物件 = 關係.objects.create(
							甲流水號=臺語編修物件, 乙流水號=釋義文字物件,
							乙對甲的關係類型=近義, 關係性質=袂當替換)
					else:
						關係物件 = 關係.objects.create(
							甲流水號=臺語編修物件, 乙流水號=釋義文字物件,
							乙對甲的關係類型=近義, 關係性質=袂當替換,詞性=詞性對照表[詞性])
		
					愛加句物件 = self.分析器.產生對齊句(標準例句.replace(例句內底漢字, 文字資料.型體, 1),
						標準標音.replace(例句內底音標, 文字資料.音標, 1))
					型體 = self.譀鏡.看型(愛加句物件)
					音標 = self.譀鏡.看音(愛加句物件)
					臺語資料腔口 = 文字資料.腔口
					臺語資料地區 = 文字資料.地區
					
					例句物件 = 文字.objects.create(來源=教育部閩南語辭典名,
						種類=種類, 腔口=臺語資料腔口, 地區=臺語資料地區, 年代=教育部閩南語辭典年代,
						組合=文字組合符號 + str(關係物件.流水號.流水號) + 文字組合符號,
						型體=型體, 音標=音標).流水號
					
					關係.objects.create(甲流水號=例句物件, 乙流水號=國語例句物件,
						乙對甲的關係類型=近義, 關係性質=會當替換)
					關係.objects.create(甲流水號=國語例句物件, 乙流水號=例句物件,
						乙對甲的關係類型=近義, 關係性質=會當替換)
	...
	def 例句變動符號(self, 標準例句, 標準標音, 臺語編修物件集):
		例句內底漢字 = None
		例句內底音標 = None
		...
		for 臺語編修物件 in 臺語編修物件集:
			文字資料 = 臺語編修物件.文字.all()[0]
			if 文字資料.腔口.startswith(閩南語):
				文字資料六 = 文字資料.型體.replace(分字符號, 分詞符號)
				文字資料七 = 文字資料.音標.replace(分字符號, 分詞符號).lstrip('0')
