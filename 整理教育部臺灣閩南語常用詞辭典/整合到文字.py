...
# 			print('主編號',主編號)
	...
	# 			print('@'.join([str(主編號), str(屬性), 漢字, 音標]))
				...
				主音 = {音標} | { 音[0] for 音 in 揣反義詞的詞音(主編號)} | { 音[0] for 音 in 揣近義詞的詞音(主編號)} | { 音[0] for 音 in 揣詞別音(主編號)}
...
# 				raise RuntimeError('{}{}{}'.format(主編號, 漢字, 錯誤))
	def 語句調整(self,主編號,資料字型體,音):
		...
		if 資料字型體 == "竹圍" and 主編號 == 36014:
			...
			音 = 'Hè-sian-ná'
		elif 資料字型體 == 'xx姊仔' and 音 == 'xxtsé--á':  # xx是會當換做名詞
			資料字型體 = '姊仔'
			音 = 'tsé--á'
...
# 							資料字型體 = "䆀猴𠢕欠數。"
		...
		return 資料字型體,音
	def 日語處理(self, 主編號, 組字式型, 音):
		if (主編號, 組字式型) in self.日語替換:
			資料字型體 = self.日語替換[(主編號, 組字式型)]
			音 = 音.replace('33', '7')
			音 = 音.replace('55', '1')
			音 = 音.replace('11', '3')
			音 = 音.replace('51', '2')
