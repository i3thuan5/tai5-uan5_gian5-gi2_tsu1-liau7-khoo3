...
def 改愛改的資料(request):
	...
	try:
		...
		字陣列 = __篩仔.篩出字物件(物件)
		詞物件.內底字 = 揣建議物件的字陣列(閩南語唸國語[0], 字陣列)
		if 詞物件.內底字==[]:
			詞物件.內底字 = 揣建議物件的字陣列(建議結果物件[0], 字陣列)
		if 詞物件.內底字==[]:
			print('字陣列內揣無，可能是外來詞')
			詞物件.內底字 = 字陣列
	except Exception as 錯誤:
		...
		詞物件.內底字 = 字陣列
	校對表格 = 文字校對表格(
		initial={'型體':譀鏡.看型(詞物件).replace('台', '臺')}, instance=愛改文字資料)
	文 = RequestContext(request, {
		'愛改資料': 愛改資料,
		'參考語句':參考語句,
...
def 是毋是攏佇辭典內底(正規物件):
	...
	for 詞物件 in 詞陣列:
		if '無佇辭典' in 詞物件.屬性 and 詞物件.屬性['無佇辭典']:
			...
			break
	return 資料攏著
def 揣建議物件的字陣列(建議物件, 字陣列):
	建議字陣列 = __篩仔.篩出字物件(建議物件)
	return 揣建議字陣列的字陣列(建議字陣列, 字陣列)
def 揣建議字陣列的字陣列(建議字陣列, 字陣列):
